...
import { 
  ...
  UserIcon, MessageSquareIcon, ClockIcon, PencilIcon, CheckIcon, UsersIcon
} from 'lucide-react'
...
type UserData = { name: string, guild_name: string | null } | null
...
export function LiveMap() {
  ...
  const [claimData, setClaimData] = useState({ name: '', type: 'main', guild_name: '' })
  ...
  // States Base (Onglets, Avis, Notes, Visites)
  const [activeTab, setActiveTab] = useState<'comments' | 'visits'>('comments')
  ...
  useEffect(() => {
    const savedUserStr = localStorage.getItem('palworld_current_user_data')
    if (savedUserStr) {
      try {
        ...
        const oldStr = localStorage.getItem('palworld_current_user')
        if (oldStr) setCurrentUserData({ name: oldStr, guild_name: null })
      }
    } else {
  ...
  const handleSelectUser = async (player: any | null, isUnknown: boolean = false) => {
    const userData: UserData = isUnknown ? { name: 'Inconnu', guild_name: null } : { name: player.name, guild_name: player.guild_name || null }
    
    if (!isUnknown && player) {
      await supabase.from('known_players').update({ claimed_by: true }).eq('name', player.name)
    }
    
    localStorage.setItem('palworld_current_user_data', JSON.stringify(userData))
    setCurrentUserData(userData)
    setIsAuthModalOpen(false)
  }

  const handleDisconnect = async () => {
    if (currentUserData && currentUserData.name !== 'Inconnu') {
      await supabase.from('known_players').update({ claimed_by: false }).eq('name', currentUserData.name)
    }
    localStorage.removeItem('palworld_current_user_data')
    localStorage.removeItem('palworld_current_user') // nettoyage ancienne version
      ...
      base.players.forEach((p: any) => {
        ...
        if (!recordedVisitsRef.current.has(visitKey)) {
          ...
          supabase.from('base_visits').insert([{
            base_id: base.id, player_name: p.name, visited_at: new Date().toISOString()
          }]).then()
        }
      })
    })
  ...
  const bossTowerMarkers = useMemo(() => points.boss_tower.map((point) => ({
    key: `boss-tower-${point[0]}-${point[1]}`, worldX: point[0], worldY: point[1], position: toScreenPercent([point[0], point[1]])
  })), [])
  ...
  const fetchBaseDetails = useCallback(async (baseId: string) => {
  ...
  useEffect(() => {
    if (!selectedBase) {
      setActiveTab('comments') // Reset tab
      return
    }
    fetchBaseDetails(selectedBase.id)
    setClaimData({ name: currentUserData?.name !== 'Inconnu' ? currentUserData?.name || '' : '', type: 'main', guild_name: currentUserData?.guild_name || '' })
  }, [selectedBase, currentUserData, fetchBaseDetails])
  ...
  const saveBase = async () => {
    ...
    const { error } = await supabase.from('player_bases').insert([{
      player_name: playerName, guild_name: guildName, base_type: baseType, 
      location_x: newBaseCoords.x, location_y: newBaseCoords.y,
      created_by: currentUserData?.name
    }])
    
    if (!error) {
      setIsAddingBase(false); setNewBaseCoords(null); setFormData({ name: '', faction: '', type: 'main', isUnknown: false })
    } else {
  ...
  const handleClaimBase = async () => {
    ...
    const { error } = await supabase.from('player_bases').update({
      ...
      guild_name: claimData.guild_name || null
  ...
  const handleRateBase = async (rating: number) => {
    if (!currentUserData || currentUserData.name === 'Inconnu' || !selectedBase) return

    const existingRating = baseRatings.find(r => r.player_name === currentUserData.name)
    ...
    if (isUpdate) {
      ...
      const { data } = await supabase.from('base_ratings').insert([{
        base_id: selectedBase.id, player_name: currentUserData.name, rating
      }]).select().single()
      if (data) setBaseRatings([...baseRatings, data])
    }

    // Créer l'événement système dans le fil de commentaires
    const { data: reviewData } = await supabase.from('base_reviews').insert([{
      base_id: selectedBase.id,
      player_name: currentUserData.name,
      content: isUpdate ? "a modifié sa note" : "a noté la base",
      is_system: true,
      rating_value: rating
    }]).select().single()

    if (reviewData) setBaseReviews(prev => [reviewData, ...prev])
  }

  const submitReview = async () => {
    if (!newReviewContent.trim() || !currentUserData || currentUserData.name === 'Inconnu' || !selectedBase) return
  ...
  const markBossDefeated = async () => {
    ...
    const newTimer = { id: crypto.randomUUID(), boss_key: `manual-boss-${Date.now()}`, name: bossName, respawn_time: respawnDate.toISOString(), notified_respawn: false }
  ...
  const handleMouseMoveOnMap = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const [worldX, worldY] = fromMapPosition([-topRatio * 256, leftRatio * 256])
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (!start) return
      if (mapPlaneRef.current) mapPlaneRef.current.style.transform = `translate(-50%, -50%) translate(${start.panX + (event.clientX - start.x)}px, ${start.panY + (event.clientY - start.y)}px) scale(${scale})`
    }
    const handleMouseUp = (event: MouseEvent) => { 
      if (dragStartRef.current) setPan({ x: dragStartRef.current.panX + (event.clientX - dragStartRef.current.x), y: dragStartRef.current.panY + (event.clientY - dragStartRef.current.y) })
  ...
  const handleMapClick = useCallback(async (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const planeRect = mapPlaneRef.current?.getBoundingClientRect()
    if (!planeRect) return
    const [worldX, worldY] = fromMapPosition([-clamp((event.clientY - planeRect.top) / planeRect.height, 0, 1) * 256, clamp((event.clientX - planeRect.left) / planeRect.width, 0, 1) * 256])
  ...
  const handleWheel = useCallback((event: React.WheelEvent<HTMLDivElement>) => {
      ...
      const ratio = (1 + newZoom * 0.45) / (1 + zoom * 0.45)
  ...
  const handleBaseClick = (e: React.MouseEvent, base: any) => {
    ...
    setPan({ x: (0.5 - mapY / 256) * 1024, y: (0.5 - (-mapX / 256)) * 1024 })
  }
  ...
  return (
    <>
      {isAuthModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <Card className="w-full max-w-md p-6 shadow-2xl border-white/10 bg-background/95 backdrop-blur-xl rounded-2xl text-center space-y-6">
            ...
            <p className="text-sm text-muted-foreground">Sélectionne ton compte pour lier tes actions. Si tu n'es pas dans la liste, connecte-toi une fois au serveur en jeu.</p>
            <div className="max-h-[300px] overflow-y-auto space-y-2 text-left custom-scrollbar p-1">
              {unclaimedPlayers.map(p => (
                <Button key={p.name} variant="outline" className="w-full justify-start h-12 flex items-center" onClick={() => handleSelectUser(p)}>
                  <UserIcon className="mr-3 h-5 w-5 text-primary" /> 
                  <div className="flex flex-col items-start leading-tight">
                    <span>{p.name}</span>
                    {p.guild_name && <span className="text-[10px] text-gray-400">Guilde: {p.guild_name}</span>}
                  </div>
                </Button>
              ))}
              {unclaimedPlayers.length === 0 && <div className="text-center p-4 text-sm text-gray-400 italic border border-white/5 rounded-lg bg-white/5">Aucun joueur non assigné trouvé.</div>}
            </div>
            <div className="pt-4 border-t border-white/10">
              <Button variant="ghost" className="w-full text-gray-400 hover:text-white" onClick={() => handleSelectUser(null, true)}>Continuer en tant qu'Inconnu</Button>
            </div>
          </Card>
        </div>
      )}
      ...
      <div className="flex flex-col h-[calc(100vh-4rem)] w-full overflow-hidden bg-background text-foreground relative font-sans">
        ...
        {isAddingBase && !newBaseCoords && <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 bg-blue-600 text-white px-6 py-2 rounded-full font-bold shadow-lg animate-pulse border border-blue-400">🖱️ Clic droit pour poser la base — (Echap pour annuler)</div>}
        {movingBaseId && <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 bg-yellow-600 text-white px-6 py-2 rounded-full font-bold shadow-lg animate-pulse border border-yellow-400">🖱️ Clic droit pour placer la base — (Echap pour annuler)</div>}
        ...
        {currentUserData && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-black/80 backdrop-blur-xl border border-white/10 px-4 py-2 rounded-full shadow-2xl">
            ...
            <div className="flex flex-col leading-tight">
              <span className="text-sm font-bold text-white">{currentUserData.name}</span>
              {currentUserData.guild_name && <span className="text-[10px] text-primary/80">{currentUserData.guild_name}</span>}
            </div>
            <button onClick={handleDisconnect} className="ml-2 text-gray-400 hover:text-red-400 transition-colors p-1" title="Changer de compte"><LogOutIcon className="h-4 w-4" /></button>
          </div>
        )}
        ...
        <div className="relative flex-1 w-full h-full overflow-hidden bg-[#1e2329]">
          <div
            ...
            style={{ overscrollBehavior: 'contain' }} onMouseDown={handleMouseDown} onMouseUp={handleMapClick} onWheel={handleWheel} onMouseMove={handleMouseMoveOnMap} onContextMenu={(e) => { if (isAddingBase || movingBaseId) e.preventDefault() }}
          >
            <div
            ...
            >
              ...
              {showFastTravels && fastTravelMarkers.map((marker) => (
                <div key={marker.key} className="absolute z-10 flex items-center justify-center" style={{ ...marker.position, transform: `translate(-50%, -50%) scale(${1 / scale})` }}><img src="/palworld-map/fast_travel.webp" alt="Fast Travel" className="h-6 w-6 object-contain drop-shadow-md" draggable={false} /></div>
              ))}
                  ...
                  <div key={base.id} className={`absolute cursor-pointer group ${isSelected ? 'z-[60]' : 'z-20'}`} style={{ ...position, transform: `translate(-50%, -50%) scale(${1 / scale})` }} onClick={(e) => handleBaseClick(e, base)}>
                    {isActive && (
                      <div className="absolute z-50 bottom-full mb-3 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/90 backdrop-blur-md px-2 py-1 rounded-md border border-green-500/50 shadow-2xl whitespace-nowrap pointer-events-none">
                        <div className="flex gap-[2px] items-end h-2"><div className="w-[2px] h-full bg-green-400 animate-[bounce_1s_infinite]" /><div className="w-[2px] h-[60%] bg-green-400 animate-[bounce_1s_infinite]" style={{ animationDelay: '150ms' }} /></div>
                        <span className="text-[10px] font-bold text-green-400 uppercase">{base.players.length} joueur{base.players.length > 1 ? 's' : ''}</span>
                      </div>
                    )}
                    {isActive && !isUnknown && <div className="absolute z-0 top-0 left-1/2 -translate-x-1/2 rounded-full animate-pulse" style={{ width: isMain ? '50px' : '30px', height: isMain ? '50px' : '30px', marginTop: isMain ? '-25px' : '-15px', boxShadow: '0 0 25px 15px rgba(34, 197, 94, 0.5)', backgroundColor: 'rgba(34, 197, 94, 0.2)' }}/>}
                    ...
                    {!isSelected && (
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 mt-2 pointer-events-none rounded-md bg-black/85 px-3 py-1.5 shadow-lg backdrop-blur-sm border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity z-30 flex flex-col items-center w-max">
                        <span className="text-xs font-bold text-white flex items-center gap-1.5">{isUnknown ? 'Base Inconnue' : base.player_name}{!isUnknown && <span className="text-gray-400 text-[10px] font-normal">({isMain ? 'Principale' : 'Secondaire'})</span>}</span>
                        {isActive && <div className="mt-1 pt-1 border-t border-white/10 w-full flex flex-col items-center"><span className="text-[10px] text-green-400 font-semibold mb-0.5">🟢 {base.players.length} Joueur(s) dans la base</span>{base.players.map((p: any) => <span key={p.name} className="text-[9px] text-gray-300">{p.name}</span>)}</div>}
                      </div>
                    )}

                    {/* FICHE D'INFORMATION COMPLETE & OPTIMISEE */}
                    {isSelected && (
                      <div 
                        className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-50 cursor-default animate-in fade-in zoom-in-95 duration-200"
                        onMouseDown={(e) => e.stopPropagation()} onMouseUp={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()} 
                        onWheel={(e) => e.stopPropagation()} // <-- EMPECHE LE DEZOOM DE LA MAP AU SCROLL
                      >
                        <Card className="w-[340px] p-4 shadow-2xl shadow-black/50 border-white/10 bg-background/95 backdrop-blur-xl rounded-xl relative flex flex-col gap-4">
                          ...
                          <div className="flex items-start gap-3 relative z-10">
                            <div className="h-10 w-10 mt-1 rounded-lg bg-primary/20 flex items-center justify-center shrink-0"><MapPinIcon className="h-5 w-5 text-primary" /></div>
                            <div className="min-w-0 pr-6">
                              <h3 className="text-base font-bold truncate flex items-center gap-1">
                                {isUnknown ? 'Base Inconnue' : base.player_name}
                                {!isUnknown && base.guild_name && <Badge variant="secondary" className="text-[9px] py-0 px-1 ml-1 bg-white/10">{base.guild_name}</Badge>}
                              </h3>
                              <p className="text-xs text-muted-foreground mb-1">{isUnknown ? 'Non revendiquée' : base.base_type === 'main' ? 'Base Principale' : 'Base Secondaire'}</p>
                              {base.created_by && <p className="text-[10px] text-gray-400 italic">Découverte par : {base.created_by}</p>}
                          ...
                          {!isUnknown && currentUserData && currentUserData.name !== 'Inconnu' && (
                            <div className="flex items-center justify-between p-2 rounded-lg bg-white/5 border border-white/5 text-sm">
                              ...
                              <div className="flex gap-1">
                                {[1, 2, 3, 4, 5].map(star => {
                                  const myRating = baseRatings.find(r => r.player_name === currentUserData.name)?.rating || 0
                                  return <button key={star} onClick={() => handleRateBase(star)} className="hover:scale-110 transition-transform"><StarIcon className={`h-4 w-4 ${star <= myRating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-500'}`} /></button>
                                })}
                              </div>
                            </div>
                          )}
                          ...
                          {isUnknown ? (
                            <div className="space-y-3 p-3 rounded-lg border border-primary/30 bg-primary/5 relative z-10">
                              ...
                              <input type="text" className="flex h-9 w-full rounded-lg border border-white/10 bg-black/40 px-3 text-xs focus:ring-2 focus:ring-primary outline-none" value={claimData.guild_name} onChange={e => setClaimData({...claimData, guild_name: e.target.value})} placeholder="Nom de guilde (Optionnel)" />
                            ...
                            <>
                              ...
                              <div className="flex border-b border-white/10">
                                <button onClick={() => setActiveTab('comments')} className={`flex-1 flex justify-center items-center gap-2 pb-2 text-xs font-bold transition-colors ${activeTab === 'comments' ? 'text-primary border-b-2 border-primary' : 'text-gray-500 hover:text-gray-300'}`}><MessageSquareIcon className="h-3 w-3" /> Avis</button>
                                <button onClick={() => setActiveTab('visits')} className={`flex-1 flex justify-center items-center gap-2 pb-2 text-xs font-bold transition-colors ${activeTab === 'visits' ? 'text-primary border-b-2 border-primary' : 'text-gray-500 hover:text-gray-300'}`}><ClockIcon className="h-3 w-3" /> Visites ({baseVisits.length})</button>
                              </div>

                              <div className="min-h-[140px] max-h-[160px] overflow-y-auto custom-scrollbar pr-1 relative z-10">
                                {activeTab === 'comments' && (
                                  <div className="space-y-2">
                                    {currentUserData && currentUserData.name !== 'Inconnu' && (
                                      <div className="flex gap-2 mb-3">
                                        <input type="text" placeholder="Laisse un avis..." className="flex h-8 w-full rounded-lg border border-white/10 bg-black/40 px-2 text-xs outline-none focus:ring-1 focus:ring-primary" value={newReviewContent} onChange={(e) => setNewReviewContent(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') submitReview() }} />
                                        <Button size="sm" className="h-8 px-2" onClick={submitReview}>OK</Button>
                                      </div>
                                    )}

                                    {baseReviews.map(review => {
                                      // EVENT SYSTEME (Notation)
                                      if (review.is_system) {
                                        return (
                                          <div key={review.id} className="flex items-center justify-center gap-1 my-1 opacity-70">
                                            <StarIcon className="h-2.5 w-2.5 text-yellow-500" />
                                            <span className="text-[9px] text-gray-400 italic text-center">
                                              <strong className="text-gray-300">{review.player_name}</strong> {review.content} <strong className="text-yellow-500">{review.rating_value}/5</strong>
                                            </span>
                                          </div>
                                        )
                                      }
                                      ...
                                      // AVIS CLASSIQUE
                                      const isOwner = review.player_name === base.player_name || (base.guild_name && currentUserData?.guild_name === base.guild_name && review.player_name === currentUserData?.name)
                                      const isMe = currentUserData && review.player_name === currentUserData.name
                                        ...
                                        <div key={review.id} className={`p-2 rounded-lg text-xs flex flex-col gap-1 ${isOwner ? 'bg-primary/10 border border-primary/30' : 'bg-white/5 border border-white/5'}`}>
                                          <div className="flex justify-between items-center">
                                            <span className={`font-bold ${isOwner ? 'text-primary' : 'text-gray-300'}`}>
                                              {review.player_name} {isOwner && <span className="text-[9px] bg-primary/20 px-1 rounded ml-1 uppercase">Proprio</span>}
                                            </span>
                                            <span className="text-[9px] text-gray-500">{new Date(review.created_at).toLocaleString('fr-FR', { timeZone: 'Europe/Paris', hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit' })}</span>
                                          </div>
                                    ...
                                    {baseReviews.length === 0 && <p className="text-[10px] text-gray-500 italic text-center py-4">Aucun commentaire.</p>}
                                  </div>
                                )}
                                ...
                                {activeTab === 'visits' && (
                                  <div className="flex flex-col gap-1.5">
                                    {baseVisits.map(visit => (
                                      <div key={visit.id} className="flex justify-between items-center text-[10px] text-gray-400 bg-white/5 p-1.5 rounded-md border border-white/5">
                                        <span className="flex items-center gap-1.5"><UserIcon className="h-3 w-3" /> {visit.player_name}</span>
                                    ...
                                    {baseVisits.length === 0 && <p className="text-[10px] text-gray-500 italic text-center py-4">Aucune visite enregistrée.</p>}
                                  </div>
                                )}
                              </div>
                            </>
                          ...
                          <div className="flex gap-2 justify-between pt-3 border-t border-white/10 relative z-10">
                            <Button variant="ghost" size="sm" className="rounded-lg hover:bg-red-500/20 text-red-400 hover:text-red-300 gap-1.5 flex-1 h-8 text-xs" onClick={() => handleDeleteBase(base.id, base.player_name)}><TrashIcon className="h-3 w-3" /> Suppr.</Button>
                            <Button variant="secondary" size="sm" className="rounded-lg gap-1.5 flex-1 bg-white/10 hover:bg-white/20 h-8 text-xs" onClick={startMovingBase}><MoveIcon className="h-3 w-3" /> Déplacer</Button>
                          </div>
                        </Card>
                      </div>
                    )}
              ...
              {showBossTowers && bossTowerMarkers.map((point) => (
                <div key={point.key} className="absolute z-20 flex flex-col items-center justify-center cursor-pointer group" style={{ ...point.position, transform: `translate(-50%, -50%) scale(${1 / scale})` }}><img src="/palworld-map/boss_tower.webp" alt="Tour" className="h-10 w-10 select-none object-contain transition-transform group-hover:scale-110 drop-shadow-xl" draggable={false} /></div>
              ))}
              ...
              {showPlayers && clusteredPlayers.map((cluster) => { 
                ...
                return (
                  <div key={cluster.id} className="absolute z-30 flex flex-col items-center group pointer-events-auto" style={{ ...position, transform: `translate(-50%, -50%) scale(${1 / scale})`, transition: 'left 5s linear, top 5s linear' }}>
                    <div className="absolute bottom-full mb-1 pointer-events-none z-10 flex flex-col items-center justify-center rounded-md border border-primary/30 bg-background/80 px-2 py-1 shadow-lg backdrop-blur-sm transition-transform duration-300 group-hover:scale-110">
                      {isGroup ? <><span className="text-[10px] font-bold text-primary mb-0.5">{cluster.players.length} Joueurs ensemble</span>{cluster.players.map(p => <span key={p.name} className="whitespace-nowrap text-[11px] font-bold text-foreground leading-tight">{p.name}</span>)}</> : <span className="whitespace-nowrap text-[11px] font-bold text-foreground drop-shadow-sm leading-tight">{cluster.players[0].name}</span>}
                    </div>
        ...
        <div className="absolute bottom-6 right-6 z-50 flex flex-col gap-2">
          <Card className="bg-background/70 backdrop-blur-xl border-white/10 shadow-2xl overflow-hidden flex flex-col">
            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-none hover:bg-white/10" onClick={() => setZoom(z => clamp(z + 1, MIN_ZOOM, MAX_ZOOM))}><ZoomInIcon className="h-5 w-5" /></Button><div className="h-[1px] w-full bg-border/50" />
            <Button variant="ghost" size="icon" className="h-10 w-10 rounded-none hover:bg-white/10" onClick={() => setZoom(z => clamp(z - 1, MIN_ZOOM, MAX_ZOOM))}><ZoomOutIcon className="h-5 w-5" /></Button><div className="h-[1px] w-full bg-border/50" />
        ...
        <div className="absolute left-6 top-6 z-50 flex hidden w-[320px] flex-col gap-6 lg:flex max-h-[calc(100%-3rem)] overflow-y-auto pointer-events-none custom-scrollbar">
          <Card className="pointer-events-auto border-white/10 bg-background/60 p-5 text-foreground shadow-2xl backdrop-blur-xl rounded-2xl">
            <h3 className="font-bold text-lg mb-4 flex items-center gap-2"><MapPinIcon className="h-5 w-5 text-primary" /> Filtres de Carte</h3>
            ...
            <div className="space-y-3">
              <Button onClick={() => setIsAddingBase(true)} disabled={isAddingBase || movingBaseId !== null} className="w-full gap-2 bg-primary/90 hover:bg-primary shadow-lg shadow-primary/20 transition-all rounded-xl disabled:opacity-50"><MapPinIcon className="h-4 w-4" /> {isAddingBase ? 'Placement en cours...' : 'Signaler une base'}</Button>
              <Button onClick={() => setIsReportingBoss(true)} className="w-full gap-2 bg-red-600/90 hover:bg-red-600 shadow-lg shadow-red-600/20 transition-all rounded-xl"><SwordsIcon className="h-4 w-4" /> Signaler un Boss vaincu</Button>
            </div>
          </Card>
          ...
          {activeBossTimers.length > 0 && (
            <Card className="pointer-events-auto border-white/10 bg-background/60 p-0 text-foreground shadow-2xl backdrop-blur-xl rounded-2xl overflow-hidden flex flex-col">
              <div className="p-4 border-b border-white/10 bg-gradient-to-r from-red-900/20 to-transparent flex items-center justify-between"><h3 className="font-bold text-sm flex items-center gap-2 text-red-400"><BellIcon className="h-4 w-4" /> Boss en attente</h3><Badge variant="outline" className="border-red-500/30 text-red-400 bg-red-500/10">{activeBossTimers.length}</Badge></div>
                ...
                {activeBossTimers.map(timer => (
                  <div key={timer.id} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5 text-sm group hover:bg-red-500/10 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-red-400 font-mono text-xs bg-red-500/10 px-2 py-1 rounded-md border border-red-500/20"><LiveCountdown targetDate={timer.respawn_time} /></span>
                      <button onClick={() => handleDeleteTimer(timer.id, timer.name)} className="opacity-0 group-hover:opacity-100 p-1 text-red-400 hover:text-red-300 transition-opacity"><TrashIcon className="h-4 w-4" /></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Card className="w-full max-w-sm p-6 shadow-2xl border-white/10 bg-background/90 backdrop-blur-xl rounded-2xl">
            ...
            <div className="space-y-4">
              <label className="flex items-center gap-3 p-3 border border-white/10 rounded-xl bg-black/20 cursor-pointer hover:bg-white/5 transition-colors">
                ...
                <div className="flex flex-col"><span className="text-sm font-medium">Base Inconnue (À claim)</span><span className="text-[10px] text-muted-foreground">Coche si tu as trouvé une base au pif</span></div>
              </label>
              {!formData.isUnknown && (
                <>
                  ...
                  <select className="flex h-11 w-full rounded-xl border border-white/10 bg-black/40 px-4 text-sm focus:ring-2 focus:ring-primary outline-none" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})}><option value="main">⭐ Base Principale</option><option value="sub_1">🏠 Base Secondaire</option></select>
                </>
              )}
              <div className="flex gap-3 justify-end pt-4 border-t border-white/10"><Button variant="ghost" className="rounded-xl hover:bg-white/5" onClick={() => { setIsAddingBase(false); setNewBaseCoords(null); setFormData({...formData, isUnknown: false}) }}>Annuler</Button><Button className="rounded-xl" onClick={saveBase} disabled={!formData.isUnknown && !formData.name}>Déployer</Button></div>
            </div>
          </Card>
        </div>
      )}
      ...
      {isReportingBoss && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
          <Card className="w-full max-w-sm p-6 shadow-2xl border-red-500/30 bg-background/90 backdrop-blur-xl rounded-2xl">
            <div className="flex items-center gap-3 mb-4"><div className="h-10 w-10 rounded-full bg-red-500/20 flex items-center justify-center"><SwordsIcon className="h-5 w-5 text-red-500" /></div><h3 className="text-xl font-bold">Signaler un Boss Vaincu</h3></div>
            <p className="text-sm text-muted-foreground mb-4">Un compte à rebours de 1 heure sera lancé et visible dans le tableau de bord.</p>
            <div className="space-y-4">
              <input type="text" className="flex h-11 w-full rounded-xl border border-white/10 bg-black/40 px-4 text-sm focus:ring-2 focus:ring-red-500 outline-none" value={bossFormName} onChange={e => setBossFormName(e.target.value)} placeholder="Ex: Jetragon, Blazamut..." autoFocus onKeyDown={(e) => { if (e.key === 'Enter') markBossDefeated() }} />
              <div className="flex gap-3 justify-end pt-4 border-t border-white/10"><Button variant="ghost" className="rounded-xl hover:bg-white/5" onClick={() => { setIsReportingBoss(false); setBossFormName('') }}>Annuler</Button><Button className="rounded-xl bg-red-600 hover:bg-red-700 text-white" onClick={markBossDefeated}>Signaler & Annoncer</Button></div>
            </div>
          </Card>
        </div>
      )}
